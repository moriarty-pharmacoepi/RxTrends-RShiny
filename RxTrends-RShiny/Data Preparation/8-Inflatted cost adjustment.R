...
July_2024_value <- df %>%
  filter(Month == "Jul 2024") %>%
  pull(VALUE)

# Using July 2024 as the reference (i.e., Jul 2024 = 100)
df <- df %>%
  mutate(CPIallitems = (VALUE / July_2024_value) * 100)
...
df <- df %>%
  mutate(`Selected Base Reference Period` = "Base July 2024=100")


print(df)


output_file_path <- "/ Replace/ with / the  actual file path/infla_Jul24Refffallitems.xlsx" 
...
df <- df %>%
  mutate(CPImeds = (VALUE / July_2024_value) * 100)
...
output_file_path <- "/ Replace/ with / the  actual file path/infla_Jul24Refffmeds.xlsx"  
write_xlsx(df, output_file_path)



PCRS_prescribing <- read_excel("/ Replace/ with / the  actual file path/PCRSunadj.xlsx")
CPIall_data <- read_excel("/ Replace/ with / the  actual file path/infla_Jul24Refffallitems.xlsx")
...
CPImeds_data <- read_excel("/ Replace/ with / the  actual file path/infla_Jul24Refffmeds.xlsx")
